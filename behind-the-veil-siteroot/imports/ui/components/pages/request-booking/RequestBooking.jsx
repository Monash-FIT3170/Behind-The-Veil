...
const RequestBooking = () => {
  ...
  const isLoadingBookingsFunc = useSubscribe(
    "artist_bookings",
    artistData?.username
  );
  ...
  const getAvailableTimes = useCallback(
    ({ date, duration, bookings }) => {
      ...
      const availableTimes = artistWorkingHours.filter((workingHour) => {
        ...
        const noClashWithExistingBookings = !confirmedBookings.some(
          (booking) => {
        ...
        const isArtistWorking = serviceHours.every((serviceHour) => {
          return Boolean(
            artistWorkingHours.find((hr) =>
              isEqual(removeMilliseconds(hr), removeMilliseconds(serviceHour))
            )
          );
        });
